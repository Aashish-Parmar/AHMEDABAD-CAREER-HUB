...
exports.submitInterview = async (req, res) => {
  try {
    if (req.user.role !== 'student') {
      return res.status(403).json({ message: 'Only students can submit interview experiences' });
    }
    ...
    res.status(500).json({ message: 'Error submitting interview', error: err.message });
  }
};
...
exports.getCompanyInterviews = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'student') {
      return res.status(403).json({ message: 'Only students can view interview experiences' });
    }
