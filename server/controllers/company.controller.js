...
exports.createCompany = async (req, res) => {
  try {
    ...
    if (user.company) {
      return res
        .status(400)
        .json({ message: "You already have a company linked to your account" });
    }
...
exports.getMyCompany = async (req, res) => {
  try {
    ...
    if (req.user.role !== "recruiter") {
      return res
        .status(403)
        .json({ message: "Only recruiters can view their company" });
    }
    ...
    if (!user.company) {
      return res
        .status(404)
        .json({ message: "No company linked to your account" });
    }

    // Get statistics
    const jobCount = await Job.countDocuments({ company: user.company._id });
    const jobIds = await Job.find({ company: user.company._id }).distinct(
      "_id"
    );
...
exports.updateCompany = async (req, res) => {
  try {
    ...
    if (req.user.role !== "recruiter") {
      return res
        .status(403)
        .json({ message: "Only recruiters can update companies" });
    }

    const { id } = req.params;
    const { companyName, description, website, logoUrl, address, techStack } =
      req.body;
    ...
    if (!user.company || user.company.toString() !== id) {
      return res
        .status(403)
        .json({ message: "You can only update your own company" });
    }
...
exports.deleteCompany = async (req, res) => {
  try {
    ...
    if (req.user.role !== "recruiter") {
      return res
        .status(403)
        .json({ message: "Only recruiters can delete companies" });
    }
    ...
    if (!user.company || user.company.toString() !== id) {
      return res
        .status(403)
        .json({ message: "You can only delete your own company" });
    }
    ...
    if (jobCount > 0) {
      return res.status(400).json({
        message:
          "Cannot delete company with active job postings. Please delete all jobs first.",
      });
    }
